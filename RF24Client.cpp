  ...
  if(u){
    if (uip_newdata()){
      ...
      if (uip_len && !(u->state & (UIP_CLIENT_CLOSE | UIP_CLIENT_REMOTECLOSED))){
		  ...
		  memcpy(u->myData + u->dataCnt, uip_appdata, dataLen);
		  u->dataCnt += dataLen;
...
finish:;
	  ...
	  //uip_send(RF24Ethernet.myDataOut,send_len);
 /*if(send_len){
   uip_send(RF24Ethernet.myData,send_len);
   uip_len = send_len;
 }*/
}
